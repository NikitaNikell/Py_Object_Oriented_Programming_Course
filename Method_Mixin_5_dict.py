class DictMixin:
    def to_dict(self):
        # Создаем пустой словарь для хранения данных объекта
        result = {}

        # Перебираем все атрибуты объекта
        for key, value in self.__dict__.items():
            # Если атрибут является экземпляром DictMixin, рекурсивно вызываем to_dict()
            if isinstance(value, DictMixin):
                result[key] = value.to_dict()
            elif isinstance(value, list):
                new_per = []
                for elem in value:
                    new_per.append(elem.to_dict())
                result[key] = new_per
            else:
                result[key] = value
